...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if isinstance(noisyDistance, int):
            for position in self.legalPositions:
                distance = manhattanDistance(position, pacmanPosition)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for position in self.legalPositions:
            ...
            ghost = self.setGhostPosition(gameState, position)
            distribution = self.getPositionDistribution(ghost)

            for i, probability in distribution.items():
                counter[i] += probability * self.beliefs[position]
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        from util import Counter, manhattanDistance, sample
        ...
        if isinstance(noisyDistance, int):
            ...
            for position in self.legalPositions:
                distance = manhattanDistance(position, pacmanPosition)
                counter[position] += emissionModel[distance] * beliefs[position]

            if any([value for value in counter.values()]):
                self.particles = [sample(counter) for i in range(self.numParticles)]
            else:
                self.initializeUniformly(gameState)
            ...
            jail = self.getJailPosition()
            self.particles = [jail for i in range(self.numParticles)]
    ...
    def elapseTime(self, gameState):
        ...
        for particle in self.particles:
            ...
            newParticles.append(sample(newPosDist))
