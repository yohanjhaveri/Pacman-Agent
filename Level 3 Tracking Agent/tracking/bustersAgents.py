...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        distribution = livingGhostPositionDistributions
        get_distance = lambda a: self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, a), goal_pos)
        prob_max = [position.argMax() for position in distribution]
        goal_pos = max(prob_max, key=lambda c: distribution[prob_max.index(c)][c])
        best_pos = min(legal, key=lambda a: get_distance(a))
